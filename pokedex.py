...
Ce fichier a été créé par Perez Lucas.

Description :
Ce script contient les fonctions nécessaires pour interagir avec la base de données des Pokémon,
y compris la recherche des attributs, des types, et des évolutions des Pokémon.
...
def recherche_pokemons():
    ...
    '''
    conn = creer_connection("pokemon.db")
    cur = conn.cursor()
    resultat = cur.execute("SELECT nom FROM pokemon ORDER BY nom ASC")
    liste_pokemon = resultat.fetchall()
...
def nombre_pokemons():
    ...
    conn = creer_connection("pokemon.db")
    cur = conn.cursor()
    resultat = cur.execute("SELECT max(id) FROM pokemon")
    nombre_pokemon = resultat.fetchall()
...
def recherche_attributs_pokemon(nom_pokemon):
    ...
    conn = creer_connection("pokemon.db")
    cur = conn.cursor()
    if len(nom_pokemon) == 1 and isinstance(nom_pokemon[0], tuple):
         resultat = cur.execute("SELECT * FROM pokemon WHERE nom = ? ORDER BY nom ASC", (nom_pokemon[0][0],))
    elif isinstance(nom_pokemon, tuple):
        resultat = cur.execute("SELECT * FROM pokemon WHERE nom = ? ORDER BY nom ASC", (nom_pokemon[0],))
    else :
        resultat = cur.execute("SELECT * FROM pokemon WHERE nom = ? ORDER BY nom ASC", (nom_pokemon,))
    detaille_pokemon = resultat.fetchall()

    if len(nom_pokemon) == 1 and isinstance(nom_pokemon[0], tuple):
         resultat2 = cur.execute("SELECT evolution_de FROM pokemon WHERE nom = ?", (nom_pokemon[0][0],))
    elif isinstance(nom_pokemon, tuple):
        resultat2 = cur.execute("SELECT evolution_de FROM pokemon WHERE nom = ?", (nom_pokemon[0],))
    else :
        resultat2 = cur.execute("SELECT evolution_de FROM pokemon WHERE nom = ?", (nom_pokemon,))

    id_evolution = resultat2.fetchall()
    if id_evolution == [] :
        id_evolution = None
    else :
        id_evolution = id_evolution[0][0]
    """if isinstance(id_evolution, list) and isinstance(id_evolution[0], tuple):
        id_evolution = id_evolution[0][0]
    else :
        id_evolution = id_evolution[0][0]"""
    class Pokemon :
        def __init__(self) :
            self.id = detaille_pokemon[0][0]
            self.name = detaille_pokemon[0][1]
            self.nom = detaille_pokemon[0][2]
            self.pv = detaille_pokemon[0][3]
            self.attaque = detaille_pokemon[0][4]
            self.defense = detaille_pokemon[0][5]
            self.attaque_speciale = detaille_pokemon[0][6]
            self.defense_speciale = detaille_pokemon[0][7]
            self.vitesse = detaille_pokemon[0][8]
            if detaille_pokemon[0][9] == None :
                self.evolution_de = 'Aucun'
            else :
                resultat3 = cur.execute("SELECT nom FROM pokemon WHERE id = ?", (id_evolution,))
                self.evolution_de = resultat3.fetchall()
                
            self.description = detaille_pokemon[0][10]
...
def alphabétique(L) :
    if not L:
        return []
    return (alphabétique([x for x in L[1:] if x <  L[0]]) + [L[0]] + alphabétique([x for x in L[1:] if x >= L[0]]))
...
def recherche_evolutions(nom_pokemon):
    ...
    try:
        ...
        # Obtenir l'ID du Pokémon de départ
        cur.execute("SELECT id FROM pokemon WHERE nom = ?", (nom_pokemon,))
        id_pokemon = cur.fetchone()
        if id_pokemon is None:
            return []

        id_pokemon = id_pokemon[0]
        
        # Obtenir les évolutions du Pokémon
        cur.execute("SELECT nom FROM pokemon WHERE evolution_de = ?", (id_pokemon,))
        evolutions = cur.fetchall()
        
        # Extraire les noms des évolutions
        liste_evolution = [evolution[0] for evolution in evolutions]
        
        # Trier la liste des évolutions par ordre alphabétique
        liste_evolution.sort()
        
        return liste_evolution
    except Error as e:
        print(f"Erreur lors de la récupération des évolutions: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def recherche_types(nom_pokemon):
    ...
    if len(nom_pokemon) == 1 and isinstance(nom_pokemon[0], tuple):
            resultat = cur.execute("SELECT COUNT(id_pokemon) FROM est_de_type WHERE id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?)", (nom_pokemon[0][0],))
            nom_pokemon = nom_pokemon[0][0]
    elif isinstance(nom_pokemon, tuple):
         resultat = cur.execute("SELECT COUNT(id_pokemon) FROM est_de_type WHERE id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?)", (nom_pokemon[0],))
         nom_pokemon = nom_pokemon[0]
    else:
        resultat = cur.execute("SELECT COUNT(id_pokemon) FROM est_de_type WHERE id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?)", (nom_pokemon,))
    nombre_de_type = resultat.fetchall()
    for i in range(nombre_de_type[0][0]) :
        if i == 0 :
            resultat2 = cur.execute("SELECT t.nom FROM type AS t JOIN est_de_type AS edt ON t.id = edt.id_type WHERE edt.id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?)", (nom_pokemon,))
        else :
            if isinstance(id_type, list) and isinstance(id_type[0], tuple):
                resultat2 = cur.execute("SELECT t.nom FROM type AS t JOIN est_de_type AS edt ON t.id = edt.id_type WHERE edt.id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?) AND edt.id_type != ?", (nom_pokemon,id_type[0][0], ))
            elif isinstance(id_type, tuple):
                resultat2 = cur.execute("SELECT t.nom FROM type AS t JOIN est_de_type AS edt ON t.id = edt.id_type WHERE edt.id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?) AND edt.id_type != ?", (nom_pokemon,id_type[0][0], ))
            else :
                resultat2 = cur.execute("SELECT t.nom FROM type AS t JOIN est_de_type AS edt ON t.id = edt.id_type WHERE edt.id_pokemon = (SELECT p.id FROM pokemon AS p WHERE p.nom = ?) AND edt.id_type != ?", (nom_pokemon,id_type[0], ))
        type_du_pokemon = resultat2.fetchall()
        resultat3 = cur.execute("SELECT id_type FROM est_de_type as edt JOIN pokemon AS p ON edt.id_pokemon = p.id WHERE p.nom = ?", (nom_pokemon,))
        id_type = resultat3.fetchall()
        liste_type.append(type_du_pokemon[0][0])
    return liste_type
